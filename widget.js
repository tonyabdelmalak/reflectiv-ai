...
// Role-play only sanitizer: strip leaked coaching/meta
function sanitizeRolePlayOnly(text){
let s = String(text || "");
s = s.replace(/<coach>[\s\S]?</coach>/gi, "");
s = s.replace(/^\s(Challenge|My\sApproach|Impact|Sales\sGuidance)\s*:\s*.$/gmi, "");
s = s.replace(/^\s(Assistant|Coach|System)\s*:\s*/gmi, "");
s = s.replace(/^\s*#{1,6}\s+.$/gmi, "");
s = s.replace(/^\s[-]\s+.$/gmi, "");
s = s.replace(/^[“"']|[”"']$/g, "");
s = s.replace(/\n{3,}/g, "\n\n").trim();
return s;
...
Sales Guidance: short, actionable, accurate guidance.
...
function populateEIOptions() { hydrateEISelects(); }
...
// mode-specific rails and prompts
...
Output ONLY the HCP’s utterance in first person. 2–6 sentences. Natural clinical dialogue.
No headings. No bullet lists. No quotes. No meta commentary. No coaching language.
...
Never output "Sales Guidance", "Challenge", "My Approach", "Impact", or any analysis headings.

Never output JSON or any "<coach>" block.

Do not summarize or label the turn. Just speak as the HCP.

Continue back-and-forth as the HCP until told to evaluate.

If asked to "Evaluate this exchange", exit role and evaluate only then.`;
...
try {
// Only inject extras in non-role-play modes
if (currentMode !== "role-play") {
...
// Role-play replies must be pure HCP
const replyText = currentMode === "role-play" ? sanitizeRolePlayOnly(clean) : clean;
