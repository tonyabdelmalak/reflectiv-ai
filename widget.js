/*
 * ReflectivEI chat widget — single initializer
 * - Modes: emotional-assessment, hiv-product-knowledge, sales-simulation
 * - Scenario picker in Sales Simulation
 * - Coach: On/Off. No "Tone". Coach section is separate, below chat.
 * - Coach parsed from <coach>{...}</coach> with scoring + heuristic fallback
 * - Optional streaming (cfg.stream=true) with Stop
 * - Minimal Markdown rendering
 * - Optional analytics POST to cfg.analyticsEndpoint
 * - No emoji, no file attach, no Theme button
 */
...
(function () {
  ...
  function heuristicCoach(conv=[], mode=currentMode){
    ...
    const ask=/commit|agree|can we|will you|let's|next step/i.test(lastUser);
    ...
    const long=lastAI.split(/\s+/).length>160, unstruct=!(/<ol>|<ul>|<h3>|<h4>|•|- |\d\./i.test(lastAI)), noCTA=!/\b(next step|commit|plan|consider|agree|schedule|start|switch)\b/i.test(lastAI);
    const worked=[]; if(q>0) worked.push("You asked a focused question."); if(val) worked.push("You referenced evidence/access/outcomes."); if(obj) worked.push("You named a barrier.");
    const improve=[]; if(q===0) improve.push("Ask 1–2 specific questions."); if(!ask&&mode==="sales-simulation") improve.push("Seek a small commitment."); if(unstruct||long) improve.push("Use concise bullets."); if(noCTA) improve.push("End with a clear action.");
    ...
    const subs={question_quality:q?4:2, objection_handling:obj?4:2, value_articulation:val?4:2, cta_clarity:noCTA?2:4, compliance:4, empathy:3};
  ...
  function buildUI(){
    ...
    // Scenario pick (only Sales Simulation)
    ...
    // --- Coach section (separate, under chat) ---
    ...
    function updateScenarioMeta(){
      ...
      const p=sc.personaKey?(personas[sc.personaKey]||{}):{};
      metaEl.innerHTML=
        `<div class="meta-card">
           ...
           <div><strong>Persona:</strong> ${esc(p.displayName||"—")}</div>
         </div>`;
    }
    function renderMessages(){
      messagesEl.innerHTML="";
      for(const m of conversation){
        const d=el("div",`message ${m.role}`); const c=el("div","content"); c.innerHTML=renderMarkdown(m.content); d.appendChild(c); messagesEl.appendChild(d);
      }
    ...
    function renderCoach(){
      ...
      const fb=last._coach, score=typeof fb.score==="number"?Math.max(0,Math.min(100,Math.round(fb.score))):null;
      const subs=fb.subscores||{};
      body.innerHTML=
        `${score!=null?`<div class="coach-score">Score: <strong>${score}</strong>/100</div>`:""}
         ${Object.keys(subs).length?`<div class="coach-subs">${Object.entries(subs).map(([k,v])=>`<span class="pill">${esc(k.replace(/_/g," "))}: ${esc(v)}</span>`).join(" ")}</div>`:""}
         <ul class="coach-list">
...
Rules: No "Tone". Be specific. Keep lists 1–3 items.` });
      ...
      try{
        ...
        if(useStream){
          ...
          stopBtn.style.display="none"; streamAbort=null; sendCoachAnalytics(assist._coach,{mode:currentMode,scenarioId:currentScenarioId,turn:conversation.length});
        }else{
          ...
          if(!r.ok){ const t=await r.text().catch(()=>"" ); throw new Error(`Upstream ${r.status}: ${t||"no body"}`); }
          ...
          conversation.push(assist); renderMessages(); renderCoach(); sendCoachAnalytics(assist._coach,{mode:currentMode,scenarioId:currentScenarioId,turn:conversation.length});
        }
      }catch(e){
  ...
  // ---------- Optional layout prune ----------
  function pruneRoadmapSection(){
  ...
  .cw .chat-messages{min-height:260px;max-height:62vh;overflow:auto;border:1px solid var(--line);border-radius:10px;padding:12px 14px;background:var(--bg);margin-bottom:8px}
  ...
  .cw .chat-input{display:grid !important; grid-template-columns:1fr auto auto; gap:8px; align-items:start; position:relative; z-index:2}
  .cw .chat-input textarea{display:block !important;width:100%;min-height:48px;max-height:220px;padding:10px;border:1px solid var(--line);border-radius:8px;resize:vertical;background:var(--bg);color:var(--fg)}

  .cw .coach-section{background:#fffbea;border:1px solid #fde68a;border-radius:10px;padding:14px;margin:10px 0 42px}
